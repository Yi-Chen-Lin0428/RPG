...
public:
    ...
    MapLayer(const tmx::Map& map, std::size_t idx)
    {
        ...
        {
            std::cout << "ERROR: Map is not orthogonal - nothing will be drawn" << std::endl;
        }
        else if (idx >= layers.size())
        {
            std::cout << "ERROR: Layer index " << idx << " is out of range, layer count is " << layers.size() << std::endl;
        }
        else if (layers[idx]->getType() != tmx::Layer::Type::Tile)
        {
            std::cout << "ERROR: layer " << idx << " is not a Tile layer..." << std::endl;
        }
